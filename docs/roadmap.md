...
当前数据准备链条已经可以输出后续指数计算需要的裁剪后 band：
...
- 输出范围
- dtype
- 输入 band 的 CRS、transform、shape 对齐检查

这一步完成后，项目就从“准备真实遥感输入”进入“生成真实遥感产品”。
...
如果 AOI GeoJSON 缺失或无效，diagnostics 会返回 `unknown` 且 `is_retriable=false`，表示当前不是可通过日期、云量或 limit 调整解决的问题。
