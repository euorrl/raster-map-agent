...
V2 可以进一步引入 job lifecycle manager；部署版可以在任务完成后保留 output 一段时间，例如 30 分钟，然后自动删除整个 job workspace。
...
如果 scene coverage 不达标，工具会在下载前短路返回 diagnostics，并删除已生成的 AOI 中间目录。
...
当前最大可下载 scene 数量限制为 20。本地内存不足时，实际可处理数量可能更低。
...
当前 metadata 顶层就是产品信息对象，不再包裹 `schema_version`、`exported_at` 或外层 `product_info`。
