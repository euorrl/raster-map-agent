...
再计算 火灾前tif - 火灾后tif 的差异 dNBR tif, 数值越大说明损毁越严重。与此同时，由于 NBR = (NIR - SWIR) / (NIR + SWIR)，而水体在NIR 与 SWIR 波段的反射值都很小，所以（NIR + SWIR）的值很小，导致水体的NBR对微小噪声会非常敏感, 因此要查看真实火灾影响图还需要辅以卫星图或 NDWI 图排除干扰水体。
