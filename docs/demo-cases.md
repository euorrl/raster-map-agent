...
NBR比较特殊，由于火灾检测需要火灾前后，因此单张tif图并不能用于火烧迹地、火灾影响和植被受损分析。这里以发生在2023年末到2024年初澳大利亚东南部的火灾为例，选择悉尼行政区为 AOI 在两个时间窗口生成的 NBR 预览图。
...
再计算 火灾前tif - 火灾后tif 的差异 dNBR tif, 数值越大说明损毁越严重。与此同时，由于 NBR = (NIR - SWIR) / (NIR + SWIR)，而水体在NIR 与 SWIR 波段的反射值都很小，所以（NIR + SWIR）的值很小，导致水体的NBR对微小噪声会非常敏感, 因此要查看真实火灾影响图还需要辅以卫星图或ndwi图排除干扰水体。
...
<table>
  ...
  <tr>
    ...
    <td><img src="materials/bing.png" alt="Sydney bing satelite image" /></td>
  </tr>
</table>
